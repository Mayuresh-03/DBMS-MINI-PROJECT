...
const saveProfileInfo = async (req, res) => {
    ...
    const { username, age, gender, address, state, country, PIN } = req.body;
...
const saveOrderDetails = async (req, res) => {
    try {
        ...
        const paymentDetails = await payment.find({ 'customer_details.customer_id': customer_id });

        if (!paymentDetails) {
            return res.status(404).json({ message:'Payment not found' });
        }
        console.log("payment datils: ",paymentDetails)
