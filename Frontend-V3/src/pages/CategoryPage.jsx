...
const CategoryPage = ({maxPrice}) => {
  ...
  const cleanPrice = (priceString) => {
    if (!priceString || typeof priceString !== 'string') {
      return 0; // Return a default value like 0 if priceString is invalid
    }
        ...
        if (response.headers['content-type'].includes('application/json')) {
            if (Array.isArray(response.data)) {
                ...
                if (prices.length > 0) {
                    ...
                    const maxPrice = maxPriceProp !== null ? maxPriceProp : Math.max(...prices); // Use passed maxPrice or calculate from data
                    setMinMaxPrice([minPrice, maxPrice]); // Set min and max price
                    setPriceRange([minPrice, maxPrice]);  // Initialize priceRange with min and max
                } else {
...
function renderStars(rating) {
  ...
  return (
    <div className="flex bg-gradient-to-r from-amber-300 via-green-400 to-emerald-300 ">
